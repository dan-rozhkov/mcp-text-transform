...
test("returns word frequency sorted by count then word", async ()=>{
    const result = await callTool({
        name: "word_frequency",
        arguments: {
            text: "Banana apple banana. Pear apple banana!"
        }
    });
    assert.equal(CallToolResultSchema.safeParse(result).success, true);
    assert.deepEqual(result.structuredContent, {
...
test("rejects missing or non-string text arguments with MCP InvalidParams", async ()=>{
    await assert.rejects(()=>callTool({
            name: "uppercase",
            arguments: {}
        }), (error)=>error instanceof McpError && error.code === ErrorCode.InvalidParams && error.message.includes("required string property"));
    await assert.rejects(()=>callTool({
            name: "uppercase",
            arguments: {
                text: 42
...
test("rejects unknown tools with MCP InvalidParams", async ()=>{
    await assert.rejects(()=>callTool({
        ...
        }), (error)=>error instanceof McpError && error.code === ErrorCode.InvalidParams && error.message.includes("Unknown tool: missing"));
});
test("selects stdio by default and HTTP via CLI arg or env var", ()=>{
    assert.equal(getTransportMode([
        "node",
        "index.js"
    ], {}), "stdio");
    assert.equal(getTransportMode([
