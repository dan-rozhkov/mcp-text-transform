...
import { CallToolResultSchema, ErrorCode, ListToolsResultSchema, McpError, } from "@modelcontextprotocol/sdk/types.js";
import { callTool, getPort, getServerName, getTransportMode, installQwenSettings, listTools, startHttpServer, } from "./index.js";
...
test("rejects missing or non-string text arguments with MCP InvalidParams", async () => {
    await assert.rejects(() => callTool({ name: "uppercase", arguments: {} }), (error) => error instanceof McpError &&
        error.code === ErrorCode.InvalidParams &&
        error.message.includes("required string property"));
    await assert.rejects(() => callTool({ name: "uppercase", arguments: { text: 42 } }), (error) => error instanceof McpError &&
        error.code === ErrorCode.InvalidParams &&
        error.message.includes("required string property"));
});
test("rejects unknown tools with MCP InvalidParams", async () => {
    await assert.rejects(() => callTool({ name: "missing", arguments: { text: "hello" } }), (error) => error instanceof McpError &&
        error.code === ErrorCode.InvalidParams &&
...
test("selects stdio by default and HTTP via CLI arg or env var", () => {
    ...
    assert.equal(getTransportMode(["node", "index.js"], { MCP_TRANSPORT: "http" }), "http");
});
test("reads server name from CLI, env, or default", () => {
    assert.equal(getServerName(["node", "index.js"], {}), "text-transform");
    assert.equal(getServerName(["node", "index.js"], { MCP_SERVER_NAME: "env-transform" }), "env-transform");
    assert.equal(getServerName(["node", "index.js", "--server-name", "cli-transform"], {
        MCP_SERVER_NAME: "env-transform",
    }), "cli-transform");
    assert.equal(getServerName(["node", "index.js", "--server-name=equals-transform"], {}), "equals-transform");
});
test("installs Qwen settings without overwriting existing MCP servers", async () => {
    ...
    await fs.writeFile(path.join(installDir, ".qwen", "settings.json"), JSON.stringify({
        theme: "dark",
        mcpServers: {
            existing: {
                command: "node",
    ...
    const settings = JSON.parse(await fs.readFile(path.join(installDir, ".qwen", "settings.json"), "utf8"));
