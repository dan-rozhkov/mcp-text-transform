...
export const tools = [
  ...
  },
  {
    name: "count",
    description:
      "Count characters, words, and lines in text, e.g. one two -> 7 characters, 2 words, 1 line.",
    inputSchema: textInputSchema,
    outputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "slugify",
    description:
      "Convert text into a URL-friendly slug, e.g. Crème brûlée & Tea -> creme-brulee-tea.",
    inputSchema: textInputSchema,
    outputSchema: textOutputSchema,
  },
  {
    name: "word_frequency",
    description:
      "Count how often each word appears, sorted by frequency, e.g. apple apple pear -> apple: 2, pear: 1.",
    inputSchema: textInputSchema,
    outputSchema: {
      type: "object",
      properties: {
...
function requireText(args: unknown) {
  if (!args || typeof args !== "object" || typeof (args as JsonObject).text !== "string") {
    throw new McpError(
...
export async function callTool(params?: ToolCallParams) {
...
function getFlagValue(argv: readonly string[], flag: string) {
...
export function hasCliFlag(argv: readonly string[], flag: string) {
  return argv.includes(flag) || argv.some((arg) => arg.startsWith(`${flag}=`));
}

export function getTransportMode(argv = process.argv, env: Env = process.env) {
...
export function getServerName(argv = process.argv, env: Env = process.env) {
...
export function getPort(env: Env = process.env) {
...
function writeJson(res: ServerResponse, statusCode: number, body: JsonObject) {
  res.writeHead(statusCode, { "content-type": "application/json" });
  res.end(JSON.stringify(body));
}

async function handleMcpHttpRequest(req: IncomingMessage, res: ServerResponse) {
...
export async function startHttpServer({ port = getPort(), host }: StartHttpServerOptions = {}) {
...
async function readSettings(settingsPath: string) {
  try {
    return JSON.parse(await fs.readFile(settingsPath, "utf8")) as JsonObject;
  } catch (error) {
...
export async function installQwenSettings({
  ...
  const existingMcpServers =
    settings.mcpServers && typeof settings.mcpServers === "object"
      ? (settings.mcpServers as JsonObject)
      : {};
...
export async function main(argv = process.argv, env: Env = process.env) {
